...
public class SimplePortSwitchingExperiment {
        ...
        for (int hourDelay = -500; hourDelay < 500; hourDelay += 100) {
            ...
            for (int run = 0; run < NUMBER_OF_RUNS; run++) {
                ...
                IndonesiaScenario scenario = yaml.loadAs(
                    new FileReader(DIRECTORY.resolve("2species.yaml").toFile()),
                    IndonesiaScenario.class
                );
                if (hourDelay < 0) {
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions()
                        .get(0)
                        .getRegulation()).getPortWaitTimes().put(
                        "Kupang",
                        Integer.toString(Math.abs(hourDelay))
                    );
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions()
                        .get(0)
                        .getRegulation()).getPortWaitTimes().put("Benoa", "0");
                } else {
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions()
                        .get(0)
                        .getRegulation()).getPortWaitTimes().put(
    ...
    public static void switching(String[] args) throws IOException {
        ...
        for (int hourDelay = 150; hourDelay < 400; hourDelay += 1) {
            ...
            FishYAML yaml = new FishYAML();
            IndonesiaScenario scenario = yaml.loadAs(
                new FileReader(DIRECTORY.resolve("port_switching.yaml").toFile()),
                IndonesiaScenario.class
            );
            ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes()
                .put("Kupang", Integer.toString(hourDelay));
            ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes()
                .put("Benoa", "0");
