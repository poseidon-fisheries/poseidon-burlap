...
package uk.ac.ox.poseidon.burlap.burlapspike;
...
import static uk.ac.ox.poseidon.burlap.burlapspike.BurlapShodan.NUMBER_OF_EPISODES;
import static uk.ac.ox.poseidon.burlap.burlapspike.BurlapShodan.STEPS_PER_LEARNING;
...
public class BurlapQuotaInfinity {
    ...
    public static void main(final String[] args) throws IllegalAccessException, NoSuchFieldException, IOException {


        Logger.getGlobal().setLevel(Level.INFO);
        final Path containerPath = Paths.get("runs", "burlap_infinity");
...
*/
    ...
    public static void sarsaRunFourier(
        ...
        @Nullable final Pair<ShodanStateOil, Action> baseline,
    ...
    public static double runSarsa(
        ...
        @Nullable final Pair<ShodanStateOil, Action> baseline,
        final HashMap<String, Object> metadata, final double initialEpsilon
        ...
        final ShodanEnvironment delegate = new ShodanEnvironment(scenario, (Steppable) simState -> {
        }, -1);
        ...
        //poliy
        final EpsilonGreedy greedy = new EpsilonGreedy(sarsaLam, initialEpsilon);
        ...
        for (; i <= NUMBER_OF_EPISODES; i++) {
            ...
            final int maxYears = 5 + random.nextInt(40);
            final int maxDays = maxYears * 365; //5 to 45 years of simulation
            final int maxMonths = (int) Math.ceil(maxDays / 30.42);
