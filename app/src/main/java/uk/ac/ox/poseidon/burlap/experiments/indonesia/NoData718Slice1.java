...
public class NoData718Slice1 {
    ...
    static {
        ...
        for (int i = 0; i < validSpecies.length; i++) {
            ...
            parameters.add(
                new SimpleOptimizationParameter(
                    "biologyInitializer.factories$" + i + ".virginRecruits",
                    recruitBounds[0],
                    recruitBounds[1]
                )
            );
            ...
            parameters.add(
                new SimpleOptimizationParameter(
                    "biologyInitializer.factories$" + i + ".cumulativePhi",
                    phi * (1 - PHI_NOISE),
                    phi * (1 + PHI_NOISE)
                )
            );
...
//     buildScenarios();
    ...
    public static Optional<Integer> runModelOnce(
        ...
        FileWriter summaryStatisticsFile, final List<AcceptableRangePredicate> predicates
    ) throws IOException {
    ...
    public static Optional<Integer> runOneScenario(
        long randomSeed,
        List<AcceptableRangePredicate> predicates,
    ...
    public static FileWriter prepareSummaryStatisticsMasterFile(
        Path directory,
        long seed,
        List<AcceptableRangePredicate> predicates
    ) throws IOException {
