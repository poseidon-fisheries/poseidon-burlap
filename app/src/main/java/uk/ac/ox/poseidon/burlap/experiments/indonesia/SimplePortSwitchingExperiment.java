    ...
    public static void twospecies() throws IOException {
        ...
        for(int hourDelay =-500; hourDelay<500; hourDelay+=100)
        {
            ...
            for (int run = 0; run < NUMBER_OF_RUNS; run++) {
                ...
                if(hourDelay<0) {
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Kupang",
                                                                                                  Integer.toString(Math.abs(hourDelay)));
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Benoa", "0");
                }
                else
                {
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Kupang",
                                                                                                  "0");
                    ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Benoa",
    ...
    public static void switching(String[] args) throws IOException {
        ...
        for(int hourDelay =150; hourDelay<400; hourDelay+=1)
        {
            ...
            IndonesiaScenario scenario = yaml.loadAs(new FileReader(DIRECTORY.resolve("port_switching.yaml").toFile()),
                                                     IndonesiaScenario.class);
            ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Kupang",Integer.toString(hourDelay));
            ((PortBasedWaitTimesFactory) scenario.getFisherDefinitions().get(0).getRegulation()).getPortWaitTimes().put("Benoa","0");
