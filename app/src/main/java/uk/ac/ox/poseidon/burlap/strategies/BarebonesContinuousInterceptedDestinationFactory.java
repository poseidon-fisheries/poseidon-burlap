...
public class BarebonesContinuousInterceptedDestinationFactory extends BarebonesContinuousDestinationFactory {
    ...
    protected ObservationExtractor[][] buildExtractors(
        ...
        final ObservationExtractor[] commonExtractor = new ObservationExtractor[2 + dummyIntercepts.length];
        commonExtractor[0] =
            buildHabitExtractor(
                discretization,
    ...
    protected double[][] buildBetas(final FishState state, final int areas, final List<Integer> validAreas) {
        //as built originally
        final double[][] originalBeta = super.buildBetas(state, areas, validAreas);
        //add all the dummies
        final double[][] beta = new double[areas][2 + dummyIntercepts.length];
        for (int i = 0; i < areas; i++) {
     ...
     */
    public void setDummyIntercepts(final double[] dummyIntercepts) {
